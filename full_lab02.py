...
def plot_decision_boundary(X, y, model, new_book=None):
    ...
    # Clear coloring for regions
    plt.contourf(xx, yy, Z, alpha=0.3, levels=np.linspace(-1, 1, 3), colors=['#FF9999','#99FF99'])
    
    # Scatter plot for existing data points
    for label, color, marker in zip([-1,1], ['red','green'], ['o','o']):
        plt.scatter(X[y==label, 0], X[y==label, 1], c=color, label=f'{"Not Popular" if label==-1 else "Popular"}', s=80, edgecolor='k', marker=marker)
    
    # Highlight new book if given
    if new_book is not None:
        pred = model.predict(new_book)
        color = 'green' if pred == 1 else 'red'
        plt.scatter(new_book[0], new_book[1], c=color, s=250, marker='*', edgecolor='k', label='New Book')
    
    # Plot decision boundary line based on weights
...
def user_input_prediction(model):
    ...
    # Show updated visualization with new book
